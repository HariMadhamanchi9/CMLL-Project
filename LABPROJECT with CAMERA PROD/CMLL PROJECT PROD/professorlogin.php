...
// Get the professor's name from the form submission
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($professor_result->num_rows > 0) {
        ...
        // Professor's name does not exist in the professors table, add it
        $add_professor_sql = "INSERT INTO professors (professor_name) VALUES ('$professorName')";
        if ($conn->query($add_professor_sql) === TRUE) {
            // Professor added successfully, redirect to professor_review.php
            header("Location: professorreview.html?name=$professorName");
            exit;
        } else {
            // Handle the case where adding professor failed
