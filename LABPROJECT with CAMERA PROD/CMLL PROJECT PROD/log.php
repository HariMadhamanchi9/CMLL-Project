...
if (isset($_POST['rNumber'])) {
    ...
    $user_sql = "SELECT * FROM users WHERE r_number = '$rNumber'";
    ...
    if ($user_result->num_rows > 0) {
        ...
        $checkin_sql = "SELECT * FROM checkins WHERE r_number = '$rNumber' AND  checkout_time = '00:00:00'";
        $checkin_result = $conn->query($checkin_sql);

        if ($checkin_result->num_rows > 0) {
            // Update checkout_time with the current time
            
            $update_sql = "UPDATE checkins SET checkout_time = '$current_time' WHERE  r_number = '$rNumber' AND checkout_time = '00:00:00'";
